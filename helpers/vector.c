...
void *vector_at(struct vector *vector, int index)
{
    ...
    return data + (index * vector->esize);
}
...
{
    char *data = (char *)vector->data;
    return data + vector->rindex * vector->esize;
}
...
{
    char *data_start = (char *)vector->data;
    char *data_end = (char *)address;
    int index = (data_end - data_start) / vector->esize;
    vector_pop_at(vector, index);
    return index;
}
...
{
    char *dst_pos = (char *)vector_at(vector, index);
    char *next_element_pos = dst_pos + vector->esize;
    char *end_pos = (char *)vector_data_end(vector);
